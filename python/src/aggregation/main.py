...
class AggregationFilter:
    ...
    def __init__(self):
        ...
        self.end_by_client: dict[str, int] = {}
    ...
    def _process_data(self, client_id, fruit, amount):
        ...
        for i in range(len(client_fruit_top)):
            if client_fruit_top[i].fruit == fruit:
                bisect.insort(client_fruit_top, client_fruit_top.pop(i) + fruit_item.FruitItem(
                    fruit, amount
                ))
                return
    ...
    def _process_eof(self, client_id):
        logging.info(f"Received EOF for {client_id}")
        self.end_by_client[client_id] = self.end_by_client.get(client_id, 0) + 1
        if self.end_by_client[client_id] < SUM_AMOUNT:
            return
